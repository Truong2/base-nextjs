...
export default function useRefinement<T>(
  ...
  { debounce, invalidate }: { debounce?: number; invalidate?: () => void } = {},
): Refinement<T> {
  const ctxRef = useRef<RefinementContext<T>>({
    callback,
    debounce,
    invalidate,
  });
...
const createRefinement = <T>(ctxRef: { current: RefinementContext<T> }) => {
  ...
  refine.invalidate = () => {
    ...
    //Call any custom invalidate function
    if (typeof ctxRef.current.invalidate === 'function') {
      ctxRef.current.invalidate();
    }
